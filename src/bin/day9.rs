...
fn file_blocks_from_string(data: String) -> FileSystem {
    ...
    for (index, character) in data.char_indices(){
        ...
        if index % 2 == 0 {
            data_blocks.extend((0..number).into_iter().map(|_| Some(File {id: (index as i32)/2})));
        } else {
            data_blocks.extend((0..number).into_iter().map(|_| None));
        }
    };
    FileSystem {memory: data_blocks}
}
...
impl FileSystem {
    ...
    fn sort_last_file_block_to_first_empty(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        let index_of_first_none: Option<usize> = self.memory.iter().position(|memory_block| memory_block.is_none());
        let index_of_final_file_block_from_right = self.memory.iter().rev().position(|memory_block| memory_block.is_some());
    ...
    fn move_whole_file_to_first_empty(&mut self, file: File) -> Result<(), Box<dyn Error>> {
        ...
        let file_memory_indices: Vec<usize> = self.memory
            .iter()
            .positions(|mem| *mem == Some(file))
            .collect();

        let file_size = file_memory_indices.len();
        let position_of_empty_slice = self.memory
            .windows(file_size)
        ...
        match position_of_empty_slice {
            Some(index) if index < file_memory_indices[0] => {
                for relative_mem_index in 0..file_size {
                    self.memory.swap(file_memory_indices[0] + relative_mem_index, index + relative_mem_index);
                };
            },
            _ => {}
        }
...
fn part2(
    ...
    if let Some(max_file_id) = file_system.memory
        .iter()
        .filter_map(|mem| *mem)
        .map(|file| file.id).max() {
